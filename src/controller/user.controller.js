...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiError(401, error?.messge || error || "Invalid refresh token");
  }
});
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  if (!isPasswordCorrect) {
    throw new ApiError(400, "Inavalidold password");
  }
...
const getUserchannelProfile = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, channel[0], "User channel etched successfully"));
});
